...
//! |x| fn countLeadingZeros(e: `T`) -> `T` | `T` is `i32`, `u32`, `vecN<i32>`, or `vecN<u32>` | The number of consecutive 0 bits starting from the most significant bit<br>        of e, when `T` is a scalar type. Component-wise when `T` is a vector. Also known as "clz" in some languages. |
//! |x| fn countOneBits(e: `T`) -> `T` | `T` is `i32`, `u32`, `vecN<i32>`, or `vecN<u32>` | The number of 1 bits in the representation of e. Also known as "population count". Component-wise when `T` is a vector. |
...
//! |x| fn extractBits(e: `T`, offset: `u32`, count: `u32`) -> `T` | `T` is `i32` or `vecN<i32>` | Reads bits from an integer, with sign extension. <br>       When `T` is a scalar type, then:<br>       <br>        w is the bit width of `T` <br>        o = min(offset, w) <br>        c = min(count, w - o) <br>        The result is 0 if c is 0. <br>        Otherwise, bits 0..c - 1 of the result are copied from bits o..o + c - 1 of e.<br>       Other bits of the result are the same as bit c - 1 of the result. <br>       <br>        Component-wise when `T` is a vector. <br>       If count + offset is greater than w, then:<br>       <br>        <br>         It is a shader-creation error if count and offset are const-expressions.<br>        <br>         It is a pipeline-creation error if count and offset are override-expressions. |
//! |x| fn extractBits(e: `T`, offset: `u32`, count: `u32`) -> `T` | `T` is `u32` or `vecN<u32>` | Reads bits from an integer, without sign extension. <br>       When `T` is a scalar type, then:<br>       <br>        w is the bit width of `T` <br>        o = min(offset, w) <br>        c = min(count, w - o) <br>        The result is 0 if c is 0. <br>        Otherwise, bits 0..c - 1 of the result are copied from bits o..o + c - 1 of e.<br>       Other bits of the result are 0. <br>       <br>        Component-wise when `T` is a vector. <br>       If count + offset is greater than w, then:<br>       <br>        <br>         It is a shader-creation error if count and offset are const-expressions.<br>        <br>         It is a pipeline-creation error if count and offset are override-expressions. |
...
//! |x| fn firstLeadingBit(e: `T`) -> `T` | `T` is `i32` or `vecN<i32>` | Note: Since signed integers use twos-complement representation,<br>the sign bit appears in the most significant bit position. |
